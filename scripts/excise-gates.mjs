 ...
 */
const XAI_SCRUB_ALLOW = {
  file: "src/lib/harness/process.ts",
  re: /for \(const k of \[[^\]]*"XAI_API_KEY"[^\]]*\]\) \{\s*delete env\[k\];/,
};
...
export function excisionGates(root) {
  ...
  for (const dir of IMPORT_ROOTS) {
    ...
    gate(
      offenders.length === 0,
      `${dir}: no import of lib/auth, lib/db, app-data or the hosted chain${offenders.length ? ` — ${offenders.join(", ")}` : ""}`,
    );
  }
  ...
  {
    const files = [
      ...SOURCE_ROOTS.flatMap((d) => walk(root, d)),
      ...SOURCE_FILES.filter(exists),
    ].filter(
      (f) =>
        !/\.test\.(ts|mjs)$/.test(f) &&
        f !== "scripts/excise-gates.mjs" &&
        f !== "scripts/prove-excise.mjs",
    );
    const offenders = [];
    for (const f of files) {
      ...
      for (const re of FORBIDDEN_SOURCE) {
        ...
        if (
          re.source === /XAI_API_KEY/.source &&
          f === XAI_SCRUB_ALLOW.file &&
          XAI_SCRUB_ALLOW.re.test(src) &&
          src.split("XAI_API_KEY").length === 2
        )
          continue;
    ...
    gate(
      offenders.length === 0,
      `src/ scripts/ desktop/ vite.config.ts: no better-auth / kysely / pg / pglite / jose / api.x.ai / XAI_API_KEY${offenders.length ? ` — ${offenders.join(", ")}` : ""}`,
    );
  }
  ...
  if (pkg) {
    ...
    gate(
      present.length === 0,
      `package.json deps: no better-auth / kysely / pg / @types/pg / pglite / jose${present.length ? ` — ${present.join(", ")}` : ""}`,
    );
    const scripts = pkg.scripts ?? {};
    gate(
      scripts.build === "vite build",
      `build is "vite build" (got ${JSON.stringify(scripts.build)})`,
    );
    gate(
      !/db:migrate|migrate\.mjs/.test(JSON.stringify(scripts)),
      "no script runs db:migrate / migrate.mjs",
    );
    for (const k of ["db:migrate", "check:auth", "preview:restart", "preview:stop"])
      gate(!(k in scripts), `script "${k}" removed`);
    gate(
      !/with-app-env/.test(JSON.stringify(scripts)),
      "no script routes through with-app-env.mjs",
    );
    ...
    gate(
      typeof scripts["prove:excise"] === "string" &&
        /prove-excise\.mjs/.test(scripts["prove:excise"]),
      "npm run prove:excise exists",
    );
  }
  ...
  // 5. __root.tsx is LocalBot-only
  {
    ...
    gate(
      !/PreviewHostBridge|preview-host-bridge/.test(rootTsx),
      "__root.tsx: no PreviewHostBridge",
    );
    gate(
      !/__grok|manifest\.webmanifest|apple-touch-icon/.test(rootTsx),
      "__root.tsx: no /__grok/* manifest or icon tags",
    );
    gate(
      /<Outlet \/>\s*<Scripts \/>/.test(rootTsx),
      "__root.tsx: plain <Outlet /> then <Scripts />",
    );
  }
  ...
  // 6. vite.config.ts is LocalBot-only, token plugin still wraps the response
  {
    ...
    gate(
      !/grokPwaPlugin|grok-pwa|appEnvPlugin|app-env-plugin|pgliteBootstrapPlugin|authPopupPlugin|migration-plan|ssrLoadModule/.test(
        vite,
      ),
      "vite.config.ts: no grok-pwa / app-env / pglite / authPopup plugins",
    );
    gate(!/serverDir/.test(vite), 'vite.config.ts: no serverDir: "./server"');
    gate(
      /import \{ sidecarTokenPlugin \} from "\.\/scripts\/sidecar-token-plugin\.mjs"/.test(vite),
      "vite.config.ts: imports sidecarTokenPlugin",
    );
  ...
  // 7. turn.ts: Safety switch reported, no hosted path
  {
    ...
    gate(
      !/execute-turn|hosted-turn|api\.x\.ai|XAI_API_KEY/.test(turn),
      "turn.ts: no hosted chain, no API key",
    );
    gate(/allowHostedDemo/.test(turn), "turn.ts: still reports the Safety switch");
    gate(
      /if \(cfg\.allowHostedDemo\) throw new Error\(HOSTED_DEMO_REFUSAL\);/.test(
        read("src/lib/runtime/harness-launch.ts"),
      ),
      "harness-launch.ts: hosted-on still refuses",
    );
  }
  ...
  // 8. the LocalBot invariants earlier stages gate
  {
    ...
    gate(
      exists("src/lib/quit-flush.ts") &&
        exists("src/lib/quit-flush-core.ts") &&
        exists("src/lib/pending-writes.ts"),
      "renderer quit-flush modules present",
    );
    gate(
