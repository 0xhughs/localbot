...
import { PluginError, pathKeyOf, pluginsAdd, pluginsInstalled, pnpmChildEnv, pnpmLookup, pnpmStatus, type RunResult, type Runner } from "./harness/plugins.ts";
import { WHISPER_BUILD_MANIFEST, seedWhisperFromResources, whisperResourceDir, whisperRuntimeAsset, verifyBuiltWhisper } from "./runtime/stt.ts";
...
describe("Stage 20 — the gates themselves catch a regression", () => {
  ...
  it("a floating pnpm pin, or none, fails", () => {
    const dir = scratch();
    editJson(dir, "package.json", (j) => ((j.devDependencies as Record<string, string>).pnpm = "^10.0.0"));
  ...
  it("mac.identity !== null fails", () => {
    const dir = scratch();
    editJson(dir, "package.json", (j) => ((j.build as { mac: { identity: unknown } }).mac.identity = "Developer ID Application: X"));
  ...
  it("plugins.ts: losing the PATH prepend, the NO_PNPM refusal, the store dir, or the bundled-first status fails", () => {
    const dir = scratch();
    edit(dir, "src/lib/harness/plugins.ts", (s) => s.replace("env[key] = current ? `${lookup.dir}${delimiter}${current}` : lookup.dir;", "/* no prepend */"));
    assert.ok(failing(dir).some((l) => l.includes("prepended to the child's PATH")));
    const dir2 = scratch();
    edit(dir2, "src/lib/harness/plugins.ts", (s) => s.replace('if (needsPnpm && pnpm.kind === "missing") throw new PluginError("NO_PNPM", pnpm.error);', ""));
    assert.ok(failing(dir2).some((l) => l.includes("throws NO_PNPM before dsh is spawned")));
    const dir3 = scratch();
    edit(dir3, "src/lib/harness/plugins.ts", (s) => s.replace('env.npm_config_store_dir = path.join(dshHome, "pnpm-store");', ""));
    assert.ok(failing(dir3).some((l) => l.includes("npm_config_store_dir")));
    const dir4 = scratch();
    edit(dir4, "src/lib/harness/plugins.ts", (s) => s.replace('run(lookup.bin, ["--version"]', 'run(process.platform === "win32" ? "pnpm.cmd" : "pnpm", ["--version"]'));
  ...
  it("stt.ts: a built branch that only verifies (back to 'compile with cmake') fails; a cmake spawn fails", () => {
    const dir = scratch();
    edit(dir, "src/lib/runtime/stt.ts", (s) => s.replace("const seed = seedWhisperFromResources({ target, asset, from: whisperResourceDir(), to: dir });\n", ""));
    assert.ok(failing(dir).some((l) => l.includes("seeds before it can say NOT_BUILT")));
    const dir2 = scratch();
    edit(dir2, "src/lib/runtime/stt.ts", (s) => s + '\nchildProcess.spawnSync("cmake", ["--build", "."]);\n');
    assert.ok(failing(dir2).some((l) => l.includes("never spawns cmake")));
    const dir3 = scratch();
    edit(dir3, "src/lib/runtime/stt.ts", (s) => s.replace('if (have.ok) return { seeded: false, reason: "already-valid", error: null };', ""));
  ...
  it("plugins.tsx: the old 'does not bundle pnpm' sentence fails", () => {
    const dir = scratch();
    edit(dir, "src/components/localbot/plugins.tsx", (s) => s.replace("Enable / Disable and this list still work.", "This packaged LocalBot does not bundle pnpm. Enable / Disable and this list still work."));
  ...
  it("packaged.mjs / main.mjs: dropping LOCALBOT_PNPM_DIR or LOCALBOT_WHISPER_DIR fails", () => {
    const dir = scratch();
    edit(dir, "desktop/packaged.mjs", (s) => s.replace("if (has(`${p.pnpmDir}/${pnpmShimName(platform)}`)) env.LOCALBOT_PNPM_DIR = p.pnpmDir;", ""));
    assert.ok(failing(dir).some((l) => l.includes("sets LOCALBOT_PNPM_DIR only when the shim exists")));
    const dir2 = scratch();
    edit(dir2, "desktop/packaged.mjs", (s) => s.replace("env.LOCALBOT_WHISPER_DIR = p.whisperDir;", "void 0;"));
  ...
  it("build-desktop.mjs: skipping the pnpm stage, the empty-PATH shim check, or the whisper stage fails", () => {
    const dir = scratch();
    edit(dir, "scripts/build-desktop.mjs", (s) => s.replace("stagePnpm({ root, stage: pnpmStageRoot, pin: pnpmPin })", "({ shim: '', cmd: '' })"));
    assert.ok(failing(dir).some((l) => l.includes("stages pnpm from the exact pin")));
    const dir2 = scratch();
    edit(dir2, "scripts/build-desktop.mjs", (s) => s.replace("stagedPnpmVersion !== pnpmPin", "false"));
    assert.ok(failing(dir2).some((l) => l.includes("empty PATH")));
    const dir3 = scratch();
    edit(dir3, "scripts/build-desktop.mjs", (s) => s.replace('whisperTarget === "darwin-arm64"', "false"));
  ...
  it("the earlier-stage invariants still flip: runAgentTurn, token gate, quit coordinator, pins, localbot-fs.mjs", () => {
    const dir = scratch();
    edit(dir, "src/components/localbot/chat.tsx", (s) => s.replace(/runAgentTurn/g, "runSomethingElse"));
    ...
    const dir4 = scratch();
    editJson(dir4, "package.json", (j) => ((j.dependencies as Record<string, string>)["@deepseek-ai/dsh"] = "^0.1.2-alpha.5"));
...
describe("Stage 20 — bundled pnpm: lookup, PATH, refusal", () => {
  it("pnpmLookup: LOCALBOT_PNPM_DIR with a shim wins everywhere; packaged without it is missing; dev without it is PATH", () => {
    ...
    assert.deepEqual(pnpmLookup({ LOCALBOT_PNPM_DIR: dir }, "win32"), { kind: "bundled", dir, bin: path.join(dir, "pnpm.cmd") });
    ...
    const missing = pnpmLookup({ LOCALBOT_PACKAGED: "1", PATH: "/usr/local/bin:/usr/bin" }, "linux");
    ...
    const broken = pnpmLookup({ LOCALBOT_PACKAGED: "1", LOCALBOT_PNPM_DIR: path.join(dir, "nope") }, "linux");
    assert.equal(broken.kind, "missing", "a LOCALBOT_PNPM_DIR without the shim is a missing bundle, not a PATH fallback");
  ...
  it("pnpmChildEnv: bundled bin dir goes first on PATH (Path on win32), store dir under DSH_HOME, nothing else touched", () => {
    const bundled = { kind: "bundled" as const, dir: "/app/resources/localbot-pnpm/bin", bin: "/app/resources/localbot-pnpm/bin/pnpm" };
    const posix = pnpmChildEnv({ PATH: "/usr/bin:/bin", HOME: "/home/e" }, "/data/dsh-home", bundled, ":");
    ...
    const win = pnpmChildEnv({ Path: "C:\\Windows\\System32" }, "C:\\Users\\e\\AppData\\Roaming\\LocalBot\\dsh-home", { ...bundled, dir: "C:\\App\\resources\\localbot-pnpm\\bin" }, ";");
    ...
    assert.equal(empty.PATH, "/app/resources/localbot-pnpm/bin", "an empty PATH becomes just the bundle");
    const viaPath = pnpmChildEnv({ PATH: "/usr/bin" }, "/data/dsh-home", { kind: "path", bin: "pnpm" }, ":");
  ...
  it("pluginsAdd in packaged mode with no bundled pnpm refuses NO_PNPM before dsh is spawned (never exit 127 from host PATH)", async () => {
    ...
    await assert.rejects(
      pluginsAdd({ dataDir: "/unused", dshHome, dshDir: path.join(root, "dsh"), nodeBin: process.execPath, env, run }, new HarnessManager(), FIXTURE),
      (err: unknown) => err instanceof PluginError && err.code === "NO_PNPM" && /never uses pnpm from PATH/.test(err.message),
    );
    assert.equal(run.calls.length, 0, "dsh must not have been spawned");
    assert.equal(fs.existsSync(path.join(dshHome, "profiles")), false, "nothing was written to the profile");
  });
  ...
  it("pluginsAdd with a bundled pnpm hands dsh a PATH that starts with the bundle and a store dir under DSH_HOME", async () => {
    ...
    const env: NodeJS.ProcessEnv = { PATH: "/usr/bin", LOCALBOT_PACKAGED: "1", LOCALBOT_PNPM_DIR: binDir, HOME: os.homedir() };
    const r = await pluginsAdd({ dataDir: "/unused", dshHome, dshDir: path.join(root, "dsh"), nodeBin: process.execPath, env, run }, null, FIXTURE);
  ...
  it("pnpmStatus: bundled shim first (absolute path), PATH only in dev mode, packaged-without-bundle never probes", async () => {
    ...
    const bundled = await pnpmStatus({ LOCALBOT_PNPM_DIR: binDir, LOCALBOT_PACKAGED: "1", LOCALBOT_DSH_NODE: process.execPath }, run, "linux");
    assert.deepEqual(bundled, { found: true, version: PNPM_PIN, source: "bundled", dir: binDir, error: null });
    ...
    assert.deepEqual(viaPath, { found: true, version: "9.9.9", source: "path", dir: null, error: null });
    ...
    const notRunning = await pnpmStatus({ LOCALBOT_PNPM_DIR: binDir, LOCALBOT_DSH_NODE: process.execPath }, broken, "linux");
  ...
  it("pluginsInstalled reports the pnpm source so the UI can say whose pnpm it is", async () => {
    ...
    fs.writeFileSync(path.join(binDir, "pnpm"), "#!/bin/sh\n", { mode: 0o755 });
    const run = fakeRunner(() => ({ code: 0, stdout: `${PNPM_PIN}\n` }));
    const r = await pluginsInstalled({ dataDir: "/unused", dshHome, dshDir: path.join(root, "dsh"), nodeBin: process.execPath, env: { LOCALBOT_PACKAGED: "1", LOCALBOT_PNPM_DIR: binDir, LOCALBOT_DSH_NODE: process.execPath }, run }, { dump: false });
    assert.deepEqual(r.pnpm, { found: true, version: PNPM_PIN, source: "bundled", dir: binDir, error: null });
    // Installed (the dump) must still work when the bundle is missing in packaged mode: only add / remove refuse.
    const dump = fakeRunner((_bin, args) => (args.includes("--dump-config") ? { code: 0, stdout: "# == dsh-base\n- id: system-prompt\n" } : { code: 1 }));
    const noBundle = await pluginsInstalled({ dataDir: "/unused", dshHome, dshDir: path.join(root, "dsh"), nodeBin: process.execPath, env: { LOCALBOT_PACKAGED: "1", HOME: os.homedir() }, run: dump }, { dump: true });
...
describe("Stage 20 — Electron main hands the sidecar the new resources", () => {
  it("packaged.mjs resource paths include the pnpm bin and the per-target whisper folder; env only when the files exist", () => {
    const p = harnessResourcePaths({ resourcesPath: "/app/resources", platform: "darwin", arch: "arm64" });
    ...
    const env = packagedHarnessEnv({ resourcesPath: "/app/resources", platform: "darwin", arch: "arm64", exists: (f) => present.has(f) });
    ...
    const noManifest = packagedHarnessEnv({ resourcesPath: "/app/resources", platform: "darwin", arch: "arm64", exists: (f) => present.has(f) && !f.endsWith("whisper-build.json") });
    assert.equal(noManifest.LOCALBOT_WHISPER_DIR, undefined, "a whisper-cli without its manifest is not offered");
    const win = packagedHarnessEnv({ resourcesPath: "C:\\App\\resources", platform: "win32", arch: "x64", exists: (f) => f.endsWith("pnpm.cmd") });
    assert.equal(win.LOCALBOT_PNPM_DIR, "C:\\App\\resources/localbot-pnpm/bin");
    assert.equal(win.LOCALBOT_WHISPER_DIR, undefined);
    const linuxNoPnpm = packagedHarnessEnv({ resourcesPath: "/app/resources", platform: "linux", arch: "x64", exists: (f) => f.endsWith("/pnpm.cmd") });
...
describe("Stage 20 — whisper seed from resources (darwin-arm64 fixtures)", () => {
  ...
  function fixture(dir: string, body: Buffer, overrides: Record<string, unknown> = {}) {
    ...
    const manifest = { release: "v1.9.2", commit: asset.source!.commit, target: "darwin-arm64", binary: "whisper-cli", sha256: sha, sizeBytes: body.length, cmake: asset.cmake, dylibs: [], ...overrides };
  ...
  it("whisperResourceDir reads LOCALBOT_WHISPER_DIR only", () => {
    ...
    assert.equal(whisperResourceDir({ LOCALBOT_WHISPER_DIR: "/app/resources/localbot-whisper/darwin-arm64/whisper" }), path.resolve("/app/resources/localbot-whisper/darwin-arm64/whisper"));
  });
  ...
  it("fresh AppData + a valid resource → copied, verified, executable; no cmake / git / network involved", () => {
    ...
    const r = seedWhisperFromResources({ target: "darwin-arm64", asset, from, to, platform: "darwin" });
    ...
    assert.equal(fs.readdirSync(to).some((n) => n.startsWith(".")), false, "no temp files left behind");
    // Second call: already valid, nothing rewritten.
    const before = fs.statSync(path.join(to, "whisper-cli")).mtimeMs;
    assert.deepEqual(seedWhisperFromResources({ target: "darwin-arm64", asset, from, to, platform: "darwin" }), { seeded: false, reason: "already-valid", error: null });
  ...
  it("a valid AppData copy is never overwritten by a different (also valid) resource", () => {
    ...
    assert.equal(crypto.createHash("sha256").update(fs.readFileSync(path.join(to, "whisper-cli"))).digest("hex"), appSha);
  });
  ...
  it("no resource → no-resource (NOT BUILT stays honest); an invalid resource is never copied", () => {
    ...
    const to = path.join(base, "app");
    assert.deepEqual(seedWhisperFromResources({ target: "darwin-arm64", asset, from: null, to, platform: "darwin" }), { seeded: false, reason: "no-resource", error: null });
    assert.equal(fs.existsSync(to), false);
    const from = path.join(base, "res");
    fixture(from, Buffer.from("#!/bin/sh\necho x\n"), { commit: "0".repeat(40) });
    const bad = seedWhisperFromResources({ target: "darwin-arm64", asset, from, to, platform: "darwin" });
    ...
    const missing = seedWhisperFromResources({ target: "darwin-arm64", asset, from: path.join(base, "nowhere"), to, platform: "darwin" });
    ...
    assert.deepEqual(seedWhisperFromResources({ target: "linux-x64", asset: linux, from, to, platform: "linux" }), { seeded: false, reason: "not-built-row", error: null });
  });
  ...
  it("the sidecar's status + ensure paths call the seed before they can say NOT_BUILT (source-locked)", () => {
    ...
    const ensure = src.slice(src.indexOf("export async function ensureWhisperRuntime"), src.indexOf("export type EnsureModel"));
    assert.ok(ensure.indexOf("seedWhisperFromResources(") < ensure.indexOf("verifyBuiltWhisper(exe, asset)"), "seed runs before the verify that decides NOT_BUILT");
    assert.match(ensure, /carries no baked whisper-cli for this host/);
    const status = src.slice(src.indexOf("export function sttStatus"), src.indexOf("// ── spawn plan"));
    assert.match(status, /seedWhisperFromResources\(\{ target, asset, from: resourceDir, to: dir \}\)/);
